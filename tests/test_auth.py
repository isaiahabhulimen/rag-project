...
def test_invalid_authentication_scheme():
    ...
    credentials = HTTPAuthorizationCredentials(scheme="Basic", credentials="some-key")
...
def test_valid_api_key():
    ...
    credentials = HTTPAuthorizationCredentials(scheme="Bearer", credentials=rag_api_key)

    result = verify_api_key(credentials)

    expected_client_id = hashlib.sha256(rag_api_key.encode()).hexdigest()
